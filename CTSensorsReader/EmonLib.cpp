...
*/
//--------------------------------------------------------------------------------------
double EnergyMonitor::calcIrms(unsigned int Number_of_Samples, long readVccCalib)
{

#if defined emonTxV3
  int SupplyVoltage = 3300;
#else
  int SupplyVoltage = readVcc(readVccCalib);
#endif
...
long EnergyMonitor::readVcc(long readVccCalib) {
  long result;
  ...
  result = ADCL;
  result |= ADCH << 8;
  result = readVccCalib / result;  //1100mV*1024 ADC steps http://openenergymonitor.org/emon/node/1186
  return result;
